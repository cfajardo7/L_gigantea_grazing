...
PAM_average <- master_PAM %>% 
  group_by(Plate_Type, Group) %>% 
  summarise(mean_PAM = mean(PAM))
```


#PAM Data Boxplot
```{r, PAM Data Boxplot}
PAM_boxplot <- ggplot(PAM_average,
                      aes(x=Plate_Type,
                          y=mean_PAM,
                          fill=Plate_Type))+
  geom_boxplot(show.legend = FALSE)+
  labs(title = "Average PAM Readings Per Plate Type",
       x = "Cage Type",
...
PAM_ave_per_plate <- master_PAM %>% 
  ...
  summarise(PAM_average = mean(PAM))
