...
tide_height <- read_csv(here("Data", "2024_05_Tide_height.csv"))
...
by_sex_norm_lmer <- lmer(delta_F0_mean_norm~Plate_Type+(1|Group), data = by_sex_Norm_PAM) 
check_model(by_sex_norm_lmer)
table(by_sex_Norm_PAM$Plate_ID)
```

#Joined PAM normalized data and Limpet Weight and Tide Height
...
factor_tide_height <- tide_height %>% 
  mutate(Group = factor(Group)) %>% 
  select(Plate_ID, Plate_Type, Group, Tide_height_avg)
norm_PAM_weight_tide <- left_join(norm_PAM_weight, factor_tide_height)
```
#Hurdle Models
```{r}
PAM_zeros_ones <- norm_PAM_weight_tide %>% 
   ungroup()%>% 
  mutate(PAM_zeros = if_else(PAM_rate < 0, 1, 0))
glmer_PAM_Zeros_ones <- glmer(PAM_zeros~Plate_Type+Tide_height_avg+(1|Group), family = "binomial", data = PAM_zeros_ones)
summary(glmer_PAM_Zeros_ones)
         
```


#Linear Models for Normalized PAm and Weight 
```{r, Linear Models for Normalized PAm and Weight2}

norm_PAM <- norm_PAM_weight %>% 
  filter(Plate_Type != "Control",
         !Group %in% c(7,9))
pam_norm_lmer <- lmer(PAM_rate~Plate_Type+(1|Group), data = norm_PAM)
anova(pam_norm_lmer)
summary(pam_norm_lmer)
check_model(pam_norm_lmer)

tide_PAM <- norm_PAM_weight_tide %>% 
  filter(Plate_Type != "Control",
         !Group %in% c(2,7,9))
pam_tide_lmer <- lmer(log(PAM_rate+10)~Plate_Type+Tide_height_avg+(1|Group), data = tide_PAM)
anova(pam_tide_lmer)
summary(pam_tide_lmer)
check_model(pam_tide_lmer)
ggsave(here("Output", "PAM_Tide_Check_Model.png"))
#sjp.lmer(pam_tide_lmer, type= "pred")
```
...
norm_chl_weight_tide <- left_join(norm_chl_weight, factor_tide_height)
```

#Linear Models for Normalized Chla and Weight and Tide Height
```{r, Linear Models for Normalized PAm and Weight}

tide_chl <- norm_chl_weight_tide %>% 
  filter(Plate_Type != "Control",
         !Group %in% 2)
chl_tide_lmer <- lmer(log(chl_rate+7)~Plate_Type+Tide_height_avg+(1|Group), data = tide_chl)
anova(chl_tide_lmer)
summary(chl_tide_lmer)
check_model(chl_tide_lmer)
ggsave(here("Output", "Chl_tide_Check_Model.png"))
```
