...
```{r, Paired Graph}
paired_graph_1 <- only_fem_male %>% 
  ggplot(aes(x = Plate_Type, 
             y = Chl_conc)) +
  stat_summary(aes(group = Plate), 
               fun.y = mean, 
               geom = "path") +
  stat_summary(aes(color = Plate_Type), 
               fun.data = mean_cl_boot) +
  stat_summary(aes(color = Plate_Type), 
               fun.y = mean, geom = "point", 
               size = 4) +
  geom_point(aes(color = Plate_Type), size = 2)

paired_graph_1

paired_graph_2 <- only_fem_male %>% 
  ggplot(aes(x = Plate_Type,
             y = Chl_conc)) + 
  geom_point(aes(color = Plate_Type))+
  geom_segment(aes(x = Plate,
                   y = Chl_conc,
                   xend = Plate,
                   yend = Chl_conc,
                   colour = "segment"))
