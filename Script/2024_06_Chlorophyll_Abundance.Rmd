...
norm_FO_var <- var.test(delta_F0_mean_norm~Plate_Type, data = by_sex_Norm_PAM)
norm_FO_var #no significant difference between variance p=0.2
...
factor_limpet_weight <- limpet_weight %>% 
  mutate(Plate_ID = factor(Plate_ID),
         Group = factor(Group))
norm_PAM_weight <- inner_join(PAM_normalized, factor_limpet_weight)
```
...
chl_normalized <- chl_abun %>% #called in df used to calculate chrorophyll abundance from raw data
  ...
  ungroup() %>% #ungroup from grouping above
  group_by(Group_control) %>% #now we will group by group control
  mutate(norm_mean_ug2_cm = mean_ug_cm2[Plate_Type=="Control"], #this normalizes the mean to the controls
         norm_sd_ug_cm2 = sd_ug_cm2[Plate_Type=="Control"]) #this normalizes the sd to the controls
```
